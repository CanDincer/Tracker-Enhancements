...
Use the maintainer's preferred **two halves of one circle**, at the same radius and center. At the tracker portrait's small size, this keeps the two pools distinct without placing a second ring over more of the portrait. The tradeoff is less angular resolution than a full circle for each pool. A fixed layout is essential: full HP with zero Temp lights the HP half only, rather than changing the health scale when Temp is gained.

![Previous 5 px strokes compared with the updated 3.5 px semicircles, with native-size examples and maximum-modifier behavior](images/v1.5-health-rings.png)
...
- Use a **3.5 px stroke on a 48 px portrait** for both fills and inactive tracks: **0.70 times the previous 5 px proposal**, within the maintainer's requested 0.60–0.80 range. Scale the stroke proportionally with portrait size, keeping it fully inside the existing footprint. Preserve the radius and small permanent gaps at the top and bottom. The mockup leaves 5 degrees at each end of each half; normalize each pool's progress across the remaining track, so 100% fills its entire available half. This is the v1.5 specification; it does not change v1.4.2 rendering.
